...
static void PWMLED_Command_Generic(PWMLED_Handle_t *this, uint8_t Command, PWMLED_Cmd_Handle_t *CommandHandle, bool ForceNow);
...
__WEAK uint16_t PWMLED_Update(PWMLED_Handle_t *this)
{
  ...
  case PWMLED_BREATH:
  {
    ...
    if (this->State.Configured != true)
    {
      ...
      TmpInhaleExhaleTicks = (this->Breath.LocalConfig.PeriodTicks - this->Breath.LocalConfig.HoldInTicks - this->Breath.LocalConfig.HoldOutTicks) / 2U;

      /* Calculate time between each breath value update. With fair rounding applied. */
      this->Breath.InhaleExhaleIncrements = (TmpInhaleExhaleTicks + (1 << (PWMLED_BREATH_TICKS_MAX_BITS - 1))) >> PWMLED_BREATH_TICKS_MAX_BITS; 
...
static void PWMLED_SetPWMDutyBreath(PWMLED_Handle_t *this, uint8_t Duty)
{
  this->PWMDuty = ((((uint32_t)this->MaxPulse * this->Breath.Value) >> PWMLED_BREATH_RESOLUTION_BITS) * (uint32_t)Duty) >> PWMLED_COLOR_RESOLUTION_BITS;
}
